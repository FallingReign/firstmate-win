...
install_cmd() {
  case "$1" in
    wezterm) echo "winget install -e --id Wez.WezTerm  # or install from https://wezterm.org/" ;;
    bash) echo "winget install -e --id Git.Git  # provides Git Bash" ;;
    node) echo "winget install -e --id OpenJS.NodeJS.LTS" ;;
    gh) echo "winget install -e --id GitHub.cli" ;;
    pi) echo "npm install -g pi" ;;
    gh-axi|chrome-devtools-axi|lavish-axi) echo "npm install -g $1 && $1 setup hooks" ;;
    treehouse) echo "echo 'Install treehouse windows-amd64 release zip from https://github.com/kunchenguid/treehouse/releases' && false" ;;
    no-mistakes) echo "echo 'Install no-mistakes windows-amd64 release zip from https://github.com/kunchenguid/no-mistakes/releases' && false" ;;
    *) return 1 ;;
  esac
}

TOOLS="bash node gh treehouse no-mistakes pi gh-axi chrome-devtools-axi lavish-axi"

tool_present() {
  case "$1" in
    wezterm) fm_wezterm_cmd >/dev/null 2>&1 ;;
...
for t in $TOOLS; do
  tool_present "$t" || echo "MISSING: $t (install: $(install_cmd "$t"))"
done
tool_present wezterm || echo "MISSING: wezterm (install: $(install_cmd wezterm))"
