...
fm_pid_identity() {
  ...
  out=$(ps -p "$pid" -o lstart= -o command= 2>/dev/null || ps -p "$pid" 2>/dev/null | tail -n +2) || return 1
  [ -n "$out" ] || return 1
  printf '%s\n' "$out" | sed 's/^[[:space:]]*//'
}
...
fm_lock_claim() {
  ...
  if ! fm_lock_points_to_owner "$lockdir" "$lockdir"; then
    fm_lock_discard_owner "$lockdir"
    return 1
  fi
...
fm_lock_try_create() {
  ...
  if fm_lock_prepare_owner "$lockdir" && fm_lock_claim "$lockdir" "$lockdir" "$allowed_steal_owner"; then
...
fm_lock_try_acquire() {
  local lockdir=$1 pid steal cur rc steal_owner steal_pid primary_owner
  FM_LOCK_HELD_PID=
  FM_LOCK_OWNER_DIR=
  ...
  if fm_pid_alive "$cur"; then
    fm_lock_release "$steal" "$steal_pid"
...
fm_lock_release() {
  local lockdir=$1 expected_pid=${2:-} pid current ownerdir
  current=${expected_pid:-${BASHPID:-$$}}
  if [ -L "$lockdir" ]; then
