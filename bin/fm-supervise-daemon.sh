...
# --- tunables ---------------------------------------------------------------
FM_SUPERVISOR_TARGET_DEFAULT="${WEZTERM_PANE:-0}"
...
#   2. $WEZTERM_PANE — inherited from firstmate's own WezTerm pane.
#   3. pane 0 fallback. The caller logs a warning if it cannot validate it.
...
# task id from a recorded pane id, or legacy "<session>:fm-<id>" target.
window_to_task() {
  local w=$1 t meta mw state
  state=$(_state_root)
  for meta in "$state"/*.meta; do
    [ -e "$meta" ] || continue
    mw=$(grep '^window=' "$meta" | cut -d= -f2- || true)
    [ "$mw" = "$w" ] && { basename "$meta" .meta; return 0; }
  done
...
window_for_task() {  # <task-key>
  local key=$1 w t
  for meta in "$(_state_root)"/*.meta; do
    [ -e "$meta" ] || continue
    w=$(grep '^window=' "$meta" | cut -d= -f2- || true)
    [ -n "$w" ] || continue
    t=$(basename "$meta" .meta)
    [ "$(_stale_key "$t")" = "$key" ] || continue
    fm_term_capture "$w" 1 >/dev/null 2>&1 && { printf '%s' "$w"; return 0; }
  done
  return 1
}
...
inject_msg() {  # <message> [state]
  ...
  fm_term_capture "$target" 1 >/dev/null 2>&1 || return 1
...
fm_super_main() {
  ...
  # Priority: FM_SUPERVISOR_TARGET override > $WEZTERM_PANE (inherited from the
  # pane that launched the daemon, normally firstmate's own) > pane 0
  ...
  if [ -z "${FM_SUPERVISOR_TARGET:-}" ]; then
    if [ -n "${WEZTERM_PANE:-}" ]; then
      ...
      target_source="FALLBACK(0)"
    fi
  fi
  if discovered=$(discover_supervisor_target); then
    : # resolved cleanly
  else
    echo "warn: could not auto-discover supervisor pane (no FM_SUPERVISOR_TARGET or WEZTERM_PANE); falling back to '$discovered' - verify this is firstmate's pane" >&2
  fi
  ...
  if ! fm_term_capture "$TARGET" 1 >/dev/null 2>&1; then
    echo "error: supervisor target '$TARGET' does not resolve to a WezTerm pane; set FM_SUPERVISOR_TARGET" >&2
