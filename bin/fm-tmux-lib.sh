...
#
# All functions are `set -u` and `set -e` safe (guarded terminal calls, explicit
...
fm_tmux_composer_state() {  # <target> -> empty|pending|unknown
  ...
  cy=$(fm_term_cursor_y "$target" 2>/dev/null) || { printf 'unknown'; return 0; }
  case "$cy" in ''|*[!0-9]*) printf 'unknown'; return 0 ;; esac
  raw=$(fm_term_capture_line "$target" "$cy" 2>/dev/null) || { printf 'unknown'; return 0; }
...
fm_pane_is_busy() {  # <target>
  local win=$1 tail40
  tail40=$(fm_term_capture "$win" 40 2>/dev/null) || return 1
...
fm_tmux_submit_enter_core() {  # <target> <retries> <enter-sleep>
  ...
  while :; do
    fm_term_send_key "$target" Enter 2>/dev/null || true
    sleep "$sleep_s"
...
fm_tmux_submit_core() {  # <target> <text> <retries> <enter-sleep> <settle>
  ...
  fm_term_send_text "$target" "$text" 2>/dev/null || { printf 'send-failed'; return 0; }
  sleep "$settle"
