...
# A fake WezTerm CLI that serves a styled composer line for the dim-aware reader and an
...
case "${1:-}" in
  list)
    printf '[{"pane_id":123,"cursor_y":%s,"tab_title":"sess:win","title":"fakepane"}]\n' "${FM_FAKE_CY:-0}"
    exit 0 ;;
  get-text)
    has_e=0
    for a in "$@"; do [ "$a" = "--escapes" ] && has_e=1; done
...
test_dim_ghost_only_composer_is_not_pending() {
  ...
  if PATH="$fb:$PATH" FM_FAKE_STYLED="$capture" FM_FAKE_CY=0 \
     fm_pane_input_pending 123; then
