...
make_fake_toolchain() {
  ...
  fm_fake_exit0 "$fakebin" node no-mistakes pi gh-axi chrome-devtools-axi lavish-axi wezterm.exe
  cat > "$fakebin/gh" <<'SH'
#!/usr/bin/env bash
...
treehouse without --lease reports an upgrade, gh auth is fine^0^-^grep^MISSING: treehouse (install: echo 'Install treehouse windows-amd64 release zip from https://github.com/kunchenguid/treehouse/releases' && false)^NEEDS_GH_AUTH
