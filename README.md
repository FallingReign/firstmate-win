...
<p align="center">
  <a
    href="https://img.shields.io/badge/platform-Windows%20%7C%20macOS%20%7C%20Linux-blue?style=flat-square"
    ><img
      alt="Platform"
...
You talk to a single agent - the first mate - and it runs the crew for you: spawning autonomous agents in terminal panes (WezTerm tabs on Windows, tmux windows elsewhere), giving each a clean git worktree, supervising them to completion, and handing you finished PRs, approved local merges, or standalone investigation reports.
...
- **A visible crew** - every crewmate lives in a terminal pane.
...
**Windows (WezTerm + Git Bash + pi):**
```sh
# Open WezTerm, then open a Git Bash tab (shell: C:\Program Files\Git\bin\bash.exe)
$ git clone <your-firstmate-origin> firstmate && cd firstmate
$ echo pi > config/crew-harness   # set pi as the crewmate harness
$ pi                               # launch firstmate; AGENTS.md takes over
```

**Linux / macOS (tmux + claude or preferred harness):**
```sh
$ git clone <your-firstmate-origin> firstmate && cd firstmate
...
# clones the project under projects/, and spawns two crewmates in terminal panes
...
# 2. git + GitHub auth
# 3. WezTerm (Windows) or tmux (Linux/macOS) - the crew lives in terminal panes (firstmate offers to install tmux if missing on non-Windows)
gh auth login
```
...
**Windows:**
```sh
# 1. Install WezTerm: winget install wez.wezterm
# 2. Ensure Git Bash is available: C:\Program Files\Git\bin\bash.exe
#    (installed with Git for Windows: winget install Git.Git)
# 3. Open WezTerm and start a Git Bash tab
git clone <your-firstmate-origin> firstmate
cd firstmate
echo pi > config/crew-harness   # tell firstmate to use pi for crewmates
pi                               # start firstmate (no tmux, no WSL needed)
```
...
On first launch the first mate detects what its required toolchain is missing or too old (WezTerm + Git Bash on Windows, or tmux on other platforms; node, gh, treehouse with durable lease support, no-mistakes, gh-axi, chrome-devtools-axi, lavish-axi), lists it with the exact install commands, and installs only after you say go.
...
**On Windows:** open WezTerm, start a Git Bash tab, `cd firstmate`, and launch your harness (e.g. `pi`). No tmux or WSL needed — crewmates spawn in new WezTerm tabs automatically.
**On Linux/macOS:** run it inside tmux for the best experience.
firstmate works from any terminal - outside the multiplexer, crewmates land in a new WezTerm tab (Windows) or a detached `firstmate` tmux session (other platforms) - but launching your harness inside WezTerm or tmux puts every crewmate pane in your own session, one per task, where you can watch the crew work in real time or type into any pane to intervene.
...
```
 ...
 └──┬──────────────┬───────────────┬───┘
    │ terminal send / status files  │
    ▼              ▼               ▼
 ┌────────┐   ┌────────┐      ┌────────┐
 │fm-task1│   │fm-task2│  ... │fm-taskN│   terminal panes you can watch
...
- **Optional secondmates** - `data/secondmates.md` records persistent domain supervisors with natural-language scopes, project clone lists, and home paths.
  `fm-home-seed.sh` provisions the isolated home, clones the listed PR-based projects into it, initializes newly cloned `no-mistakes` projects, copies the charter to `data/charter.md`, and `fm-spawn.sh --secondmate` launches it through the same terminal pane and status-file path as any direct report.
...
- **Restart-proof** - all state lives in terminal panes, status files, local markdown under `data/`, `data/secondmates.md`, and persistent secondmate homes.
...
| `fm-tmux-lib.sh`         | Compatibility shim (WezTerm + Git Bash on Windows) wrapping `fm-terminal-lib.sh`; provides shared pane primitives for busy detection, dim-ghost-aware and border-aware composer detection, and verified submit retry |
...
FM_BUSY_REGEX='esc (to )?interrupt|Working\.\.\.'   # busy-pane signatures, shared by watcher and terminal helper
...
FM_SUPERVISOR_TARGET=firstmate:0   # supervisor pane target (override; auto-discovers from $WEZTERM_PANE / $TMUX_PANE)
...
tests/fm-fleet-sync.test.sh               # no-arg sweep scope: $PROJECTS/* directory sweep, path:-registered project outside $PROJECTS, dedupe, local-only safety via registry name
