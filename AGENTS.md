...
**Exception — pi harness (no tracked background):** pi has no `run_in_background` facility; any child backgrounded with `&` inside a tool call is reaped when the call returns. Use `bin/fm-base-waker.sh &` instead of `fm-watch-arm.sh`. The base waker runs as an orphaned background process: it loops, and on each wake injects the reason line verbatim into your pane using the shared `fm-tmux-lib.sh` inject primitive (with the same composer/busy guards as `fm-send.sh`), then re-arms automatically. You never need to re-arm explicitly on pi; just start the base waker once and let it run. The `WEZTERM_PANE` env var inherited from your session provides the inject target; override with `FM_SUPERVISOR_TARGET` if needed. The watcher's singleton lock and liveness beacon (`state/.last-watcher-beat`) are maintained exactly as on other harnesses — `fm-guard.sh` and `fm-watch-arm.sh --restart` work the same way. When the base waker dies (e.g. after a session restart), start a fresh one with `bin/fm-base-waker.sh &`.
...
bin/fm-base-waker.sh &    # pi only: orphaned loop that injects each wake into your pane and re-arms
```
