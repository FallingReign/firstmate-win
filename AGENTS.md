...
The terminal pane for a task is always named `fm-<id>` (WezTerm tab title on Windows; tmux window name elsewhere).
...
A harness value may carry an optional model as `<adapter>:<model>` (e.g. `pi:sonnet`); `fm-spawn.sh` launches the adapter with its `--model` flag and records both `harness=` and `model=` in the task's meta.
Secondmate spawns take their harness/model from `config/secondmate-harness` (resolve with `bin/fm-harness.sh secondmate`), which falls back to the crew harness when absent, empty, or `default` - this is how firstmate stays on a smart model while secondmates and their crew run a cheaper one such as sonnet.
...
4. Use the `window=` values from this home's `state/*.meta` files as the live direct-report set, then check those terminal panes.
   Do not sweep every `fm-*` pane across all windows/sessions during recovery; another firstmate home's child panes may share that namespace and are not this home's orphans.
...
All truth lives in terminal panes, state files, data/backlog.md, data/secondmates.md, persistent secondmate homes, and treehouse; your conversation memory is a cache.
...
For ship and scout tasks, the script creates the window (a WezTerm pane on Windows; in your current tmux session or a dedicated `firstmate` session elsewhere), runs `treehouse get`, waits for the worktree subshell, installs the turn-end hook, records `state/<id>.meta`, and launches the agent with the brief.
...
**Exception — pi harness (no tracked background):** pi has no `run_in_background` facility; any child backgrounded with `&` inside a tool call is orphaned (adopted by init, PPID=1) when the tool-call shell exits, verified empirically on Git Bash/Cygwin. Use `nohup bin/fm-base-waker.sh >/dev/null 2>&1 &` instead of `fm-watch-arm.sh`. The base waker runs as an orphaned background process: it loops, and on each wake injects the reason line verbatim into your pane using the shared `fm-tmux-lib.sh` inject primitive (with the same composer/busy guards as `fm-send.sh`), then re-arms automatically. The `nohup` additionally protects against SIGHUP if the WezTerm session closes. You never need to re-arm explicitly on pi; just start the base waker once and let it run. The `WEZTERM_PANE` env var inherited from your session provides the inject target; override with `FM_SUPERVISOR_TARGET` if needed. The watcher's singleton lock and liveness beacon (`state/.last-watcher-beat`) are maintained exactly as on other harnesses — `fm-guard.sh` and `fm-watch-arm.sh --restart` work the same way. When the base waker dies (e.g. after a session restart), start a fresh one with `nohup bin/fm-base-waker.sh >/dev/null 2>&1 &`.
...
nohup bin/fm-base-waker.sh >/dev/null 2>&1 &  # pi only: start the orphaned loop waker
```
...
The terminal multiplexer (WezTerm on Windows, tmux elsewhere) is the ground truth.
