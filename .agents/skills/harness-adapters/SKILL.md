...
A harness value may carry an optional model as `<adapter>:<model>`, e.g. `pi:sonnet` or `pi:github-copilot/claude-sonnet-4.6`.
`fm-spawn.sh` splits the token, launches the adapter with its `--model` flag, and records both `harness=` and `model=` in `state/<id>.meta`.
A bare adapter name with no `:` pins no model and reproduces the prior behavior byte-for-byte.
Secondmate spawns resolve their harness/model from `config/secondmate-harness` (via `fm-harness.sh secondmate`), falling back to the crew resolution when that file is absent, empty, or `default`.
This is how firstmate stays on a smart model while secondmates and their crew run a cheaper one such as sonnet.
Model pinning rides the adapter's own `--model` flag, so verify a new adapter's model flag empirically before relying on it, exactly as with any other adapter fact.
...
| Exit command | `/quit` |
| Interrupt | single Escape |
| Watcher arm | `nohup bin/fm-base-waker.sh >/dev/null 2>&1 &` (see below) |
...
**Watcher delivery on pi.** Pi has no `run_in_background` facility: any child backgrounded with `&` inside a pi tool call's non-interactive shell is orphaned and adopted by init (PPID=1) when that shell exits — verified empirically on Git Bash/Cygwin — so `fm-watch-arm.sh`'s exit-and-notify model cannot reach pi. Instead, start `nohup bin/fm-base-waker.sh >/dev/null 2>&1 &` once at session start (or after a restart). This orphaned process persists across tool calls under init, runs `fm-watch.sh` in a loop, and on each wake injects the reason line verbatim into your pane using the shared `fm-tmux-lib.sh` inject primitive — same composer/busy guards as `fm-send.sh`, so it never lands on a mid-turn or half-typed line. The `nohup` additionally protects against SIGHUP if the WezTerm session closes. Your pane receives the injected reason as if the captain typed it; handle it exactly as a watcher wake (drain `fm-wake-drain.sh`, act on the reason, continue). No per-wake re-arm is needed on pi; the base waker loops internally. `WEZTERM_PANE` inherited from your session is the inject target; override with `FM_SUPERVISOR_TARGET`. The watcher singleton lock, liveness beacon, and `fm-guard.sh` all work identically to other harnesses. If the base waker dies, restart it with `nohup bin/fm-base-waker.sh >/dev/null 2>&1 &`.
