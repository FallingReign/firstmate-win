...
Secondmate spawns pick their harness and model from `config/secondmate-harness` in the main firstmate home (resolved by `fm-harness.sh secondmate`), falling back to the crew harness when it is absent, empty, or `default`.
The value is an `<adapter>[:<model>]` token, e.g. `pi:github-copilot/claude-sonnet-4.6`, which keeps secondmates on a cheaper model while firstmate stays sharp.
A secondmate is itself a firstmate that spawns its own crew, so to push its crew onto the same cheap model set the seeded home's own `config/crew-harness` to the matching token.
